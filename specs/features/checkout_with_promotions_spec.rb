...
describe 'Checkout with' do
  def products
    @products ||= {
      card_holder: Product.new(code: '001', name: 'Travel Card Holder', price: 9.25),
  ...
  def rules
    @rules ||= {
      ...
      card_holder_discount: PerItemQuantityDiscount.new(
        product_code: '001',
  ...
  it '10% discount when total basket value is greater than £60' do
    ...
    checkout.scan(products[:card_holder])
  ...
  it 'prices of £8.50 per card holder when 2 or more are purchased' do
    checkout = Checkout.new([rules[:card_holder_discount]])
    checkout.scan(products[:card_holder])
    checkout.scan(products[:tshirt])
    checkout.scan(products[:card_holder])
  ...
  it 'gives 10% discount and reduce prices for each card holder when basket total is over £60 and there are 2 card holders' do
    checkout = Checkout.new([rules[:card_holder_discount], rules[:basket_discount]])
    checkout.scan(products[:card_holder])
    checkout.scan(products[:cufflinks])
    checkout.scan(products[:card_holder])
    checkout.scan(products[:tshirt])
